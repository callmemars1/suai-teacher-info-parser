...
NpgsqlConnection conn = new(myDB);
NpgsqlCommand cmd = new();
conn.Open();
cmd.Connection = conn;

#region academic_degrees
var degree_id_discription_list = (from p in JsonSerializer.Deserialize<Prepod[]>(json)
                                  where p.degree_id != "0"
...
var at_id_discription_list = (from p in JsonSerializer.Deserialize<Prepod[]>(json)
                              where p.at_id != "0"
                              select p.at_id + "\n" + p.at_name);
...
var values = JsonSerializer.Deserialize<Prepod[]>(json);
...
var posotions_list = (from p in JsonSerializer.Deserialize<Prepod[]>(json)
                      select p.works);
...
Dictionary<string, string> positions = new();

foreach (var item in posotions_list)
{
    foreach (var work in item)
    {
        positions.TryAdd(work.depname, work.post);
...
{
    cmd.CommandText = $"INSERT INTO positions(name, department_id) VALUES ('{item.Value}', (select id from departments where name = '{item.Key}'))";
    cmd.ExecuteNonQuery();
}
...
var unparsed = JsonSerializer.Deserialize<Prepod[]>(json);

//var class_rooms_list = (from p in JsonSerializer.Deserialize<Prepod[]>(json)
//                        select p.auditorium);
...
foreach (var item in unparsed)
{
    ...
    cmd.CommandText = $"INSERT INTO persons(id, first_name, second_name, last_name) VALUES ({item.id}, '{item.firstname}', '{item.lastname}', '{item.middlename}')";
    cmd.ExecuteNonQuery();
